  ...
  if (dbUnavailable) {
    return (
      <section className="card">
        ...
        <p className="muted">Lore & Memories is not available yet on this environment. Apply migration 0017_shared_posts.sql.</p>
      </section>
    );
  }
    ...
    <div className="stack">
      <PageTopRow
        items={[
          ...
          { href: `/lore-memories/${id}`, label: post.title || 'Untitled' },
        ]}
        right={
              ...
              return (
                <div key={c.id} className="reply-item">
                  ...
                  <CommentActions
                    ...
                    commentBody={c.body}
                    replyHref={`/lore-memories/${id}?replyTo=${encodeURIComponent(c.id)}#comment-form`}
                  />
                </div>
              );
            })
