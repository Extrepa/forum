    ...
    post = await db
      .prepare(
        `SELECT posts.id, posts.type, posts.title, posts.body, posts.image_key, posts.is_private,
         ...
         WHERE posts.id = ? AND posts.type = 'nostalgia'`
      )
      .bind(params.id)
      .first();
  ...
  if (dbUnavailable) {
    return (
      <section className="card">
        <h2 className="section-title">Nostalgia</h2>
        <p className="muted">Nostalgia is not available yet on this environment. Apply migration 0017_shared_posts.sql.</p>
      </section>
    );
  }
              ...
              return (
                <div key={c.id} className="reply-item">
                  ...
                  <CommentActions
                    ...
                    commentBody={c.body}
                    replyHref={`/nostalgia/${post.id}?replyTo=${encodeURIComponent(c.id)}#comment-form`}
                  />
                </div>
              );
            })
