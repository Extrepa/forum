...
export default async function BugDetailPage({ params, searchParams }) {
  ...
  if (dbUnavailable) {
    return (
      <section className="card">
        ...
        <p className="muted">Bugs is not available yet on this environment. Apply migration 0017_shared_posts.sql.</p>
      </section>
    );
  }
  ...
  if (!post) {
    return (
      <section className="card">
        ...
        <p className="muted">This bug report does not exist (or it is members-only).</p>
      </section>
    );
  }
    ...
    <div className="stack">
      <PageTopRow
        items={[
          ...
          { href: `/bugs/${post.id}`, label: post.title || 'Bug report' },
        ]}
        right={
          (isAdmin || canEdit) ? (
            <PostActionMenu
              ...
              editModal={
                <section className="card">
                  ...
                  <PostEditForm
                    ...
                    showImage={true}
                    onSuccess={() => {
                      // Page will refresh automatically on form submit redirect,
