...
export async function getSessionUser() {
  ...
  try {
    ...
    const user = await db
      .prepare(
        'SELECT id, username, role, email, phone, password_hash, must_change_password, notify_email_enabled, notify_sms_enabled, notify_rsvp_enabled, notify_like_enabled, notify_update_enabled, notify_mention_enabled, notify_reply_enabled, notify_comment_enabled, notify_admin_new_user_enabled, notify_admin_new_post_enabled, notify_admin_new_reply_enabled, ui_lore_enabled, ui_color_mode, ui_border_color, ui_invert_colors, default_landing_page, preferred_username_color_index, avatar_key, avatar_state FROM users WHERE session_token = ?'
      )
      .bind(token)
      .first();
    if (user) {
